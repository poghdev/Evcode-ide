...
/// Teleport overlay (Ctrl+P) — центральное окно fuzzy search
pub fn draw_teleport(f: &mut Frame, area: Rect, app: &App, colors: &Colors) {
    ...
    // Поле ввода (строка 1 внутри блока)
    let input_area = Rect {
        x: popup_area.x + 1,
        y: popup_area.y + 1,
...
pub fn draw_diff_view(f: &mut Frame, area: Rect, app: &App, colors: &Colors) {
    ...
    let para = Paragraph::new(lines)
        .block(block)
        .scroll((0, 0)); // Можно добавить логику скролла
