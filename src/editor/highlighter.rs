...
fn tokenize<'a>(line: &'a str, lang: Lang) -> Vec<Token<'a>> {
    ...
    while i < len {
        let Some((c, c_len)) = next_char_at(line, i) else {
            break;
        };
        ...
        if c == '"' || c == '\'' {
            ...
            let start = i;
            i += c_len;
            while let Some((ch, ch_len)) = next_char_at(line, i) {
                i += ch_len;
                if ch == '\\' {
                    if let Some((_, escaped_len)) = next_char_at(line, i) {
                        i += escaped_len;
                    }
                    continue;
                }
            ...
            tokens.push(Token {
                text: Cow::Borrowed(&line[start..i.min(len)]),
        ...
        if c.is_ascii_digit() {
            ...
            tokens.push(Token {
                text: Cow::Borrowed(&line[start..i]),
        ...
        if matches!(c, '(' | ')' | '[' | ']' | '{' | '}') {
            tokens.push(Token {
                text: Cow::Borrowed(&line[i..i + c_len]),
        ...
        if c.is_alphabetic() || c == '_' {
            ...
            let next_non_ws = line[i..].chars().find(|ch| *ch != ' ' && *ch != '\t');
...
fn next_char_at(line: &str, byte_idx: usize) -> Option<(char, usize)> {
    line.get(byte_idx..)
